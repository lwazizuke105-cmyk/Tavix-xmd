...
if (global.cartoonNumHandler) {
    ...
    if (handled) return;
            ...
            if (quotedText.includes("TAVEX-MD SEARCH") && /^[0-9]+$/.test(replyText)) {
...
const arabianCtxGlobal = {
  ...
  forwardedNewsletterMessageInfo: {
    ...
    newsletterName : 'TAVEX MD',
    serverMessageId: 143,
  },
  externalAdReply: {
    title                 : 'BY LWAZI COOL BOY',
...
case 'yta': {
    try {
        ...
        const captionMsg = `Tavex md Music System* ✨\n\n📌 *Title:* ${songTitle}\n💿 *Quality:* 320kbps Ultra-High Quality\n🚀 *Status:* downloading...`;
...
case 'wormgpt': {
    try {
        ...
        if (response.data) {
            ...
            if (aiReply) {
                const finalMessage = `*TAVEX DARK GPT*\n\n` +
                                     `${aiReply}\n\n` +
                                     `> *BY LWAZI COOL BOY*`;
